...
class classifyImage:
    ...
    def preprocessImage(self):
        image = Image.fromarray(self.image).convert('RGB')
    ...
    def classPrediction(self):
        ...
        objectProbability = {self.imageCategories[classIdx.item()]: probabilities[0, classIdx.item(
        )].item() for (_, classIdx) in enumerate(sortedProba[0, :])}
        relevantPrediction = {category: probability for category,
                              probability in objectProbability.items() if probability >= self.threshold}
