We reused some parts of the previuos team's code and changed for our specific need . Specifically, these 3 files were created and used in our project:

1) src.preprocessing.truncate_single_logo_from_path_to_xml_string.ipynb
2) src.preprocessing.sort_paths.ipynb
3) src.preprocessing.svg_truncation.py

"src.preprocessing.svg_truncation.py" contains all code from 3 files. Refer to it for more information about modules used. Otherwise, the rest of the repository remains unchanged, so below we provide documentation from previous team's repository
