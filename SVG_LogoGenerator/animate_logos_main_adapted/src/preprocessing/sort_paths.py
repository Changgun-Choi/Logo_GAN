...
class Selector:
    ...
    @staticmethod
    def sort_by_relevance(path_selection_folder, excluded_paths, nr_paths_trunc= 20, coverage_percent = 0.98) :
    ...
    def select_paths(self, svgs_folder, excluded_paths):
        ...
        for i, logo in enumerate(logos):
            ...
            sorted_ids, sorted_mses, missed_relevant_scores, missed_relevant_paths = self.sort_by_relevance(f'{self.dir_path_selection}/{logo}', excluded_paths)
    ...
    def truncate_svgs(self, svgs_folder, logos=None, excluded_paths=[], nr_paths_trunc=20, coverage_percent=0.98):
        ...
        for i, logo in enumerate(logos):
            ...
            sorted_ids, _, _, _ , kept_paths, total_paths, coverage_percent = self.sort_by_relevance(f'{self.dir_path_selection}/{logo}', excluded_paths, nr_paths_trunc, coverage_percent)
            try:
        ...
        logger.info(f'Time: {datetime.now() - start}')
        logger.info(f"Kept {number_of_kept_paths} out of total {number_of_total_paths} to cover {coverage_percent*100} percent of MSE ")
...
def _get_number_of_paths_for_truncation(ordered_relevance_scores_list, coverage_percent):
    ...
    coverage_percent - is a float mse threshold between 0 and 1(e.g: 0.60, 0.999)
    ...
    for i, mse in enumerate(ordered_relevance_scores_list):
        elements_sum = elements_sum + mse
        if elements_sum > coverage_percent*sum_list:
            logger.info(f"number of path to truncate: {i + 1} out of {len(ordered_relevance_scores_list)}")
